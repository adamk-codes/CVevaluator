    ...
    public SecurityFilterChain filterChain(
        ...
        return http
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        // The recruiter's list of every CV on one posting. Role
                        // gated here rather than left to anyRequest().authenticated(),
                        // which is what it would otherwise have fallen through to:
                        // this returns every candidate's filename and status on the
                        // job, so "any signed-in user" would let one candidate
                        // enumerate the competition. Ownership of the job is checked
                        // in the handler, since it needs the row.
                        ...
                        // Neither role alone is the rule, so authorization here is
                        // ownership rather than role, and it lives in the handler -
                        // yours if you submitted it, or if you posted the job.
                        .requestMatchers(HttpMethod.GET, "/api/jobs/*/applications/*")
                                .authenticated()
